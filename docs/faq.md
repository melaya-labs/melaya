...
From safest to most direct: **Paper (sim broker)** — synthetic fills against the live tape, no venue or capital; **Managed strategies** — launch a `custom` Rhai script or a full agentic crew and the engine runs the loop with server-side SL/TP and approval gating; **Direct live trading** — the `trade` plane places, amends, and cancels real orders and reads balances/positions/fills, one call shape across every venue. The first two need only your `mk_` key.
...
Instead of hand-tuning, the **AI optimizer** proposes parameter sets, evaluates each by backtest, and surfaces the strongest candidates for you to **approve** before they touch a live run — optimization with a human gate, not a black box.

### Does Melaya offer real-time market data streaming?

Yes — WebSocket streams for ticker, order book, OHLCV, public trades, and liquidations (filterable to one venue or consumed as a cross-exchange firehose), plus authenticated private streams for account balance/positions/orders/fills and strategy events. The ticker stream fires only when the normalized ticker advances — no duplicate frames.
...
Yes. Run local providers (Ollama, LM Studio) through a lightweight runner on your own machine. Prompts and your model key stay local, and there's no per-token LLM cost — the platform still tracks token usage for observability. Fully-local crews are exempt from the daily cost cap.
...
In local-runner mode the LLM call goes directly from your machine to your provider (or stays entirely on-device with Ollama/LM Studio) — Melaya never sees those prompts or your model key. Order intent and fills still pass through the approval gate and audit chain so the safety and compliance story holds.
...
Three foundational layers plus trading-specific rails: **(1)** scoped tools and permissions — an agent can only do what it's granted; **(2)** encrypted per-user credential isolation — agents never see raw secrets; **(3)** human-in-the-loop approval gates on risky tools — the run pauses for an operator decision, and every decision is audited. For trading, add write caps, order quotas, cost caps, loss circuit breakers, an egress allowlist, a server-enforced tool allowlist, and the paper-soak gate.
...
Yes. Messaging tools (Telegram, Slack, Discord, WhatsApp/SMS/voice, email, Twitter) let a crew self-report — e.g. the Execution seat pings a channel after a fill, or a research crew sends a daily briefing. You can run "analysis-only" crews with no write/order tools at all.
